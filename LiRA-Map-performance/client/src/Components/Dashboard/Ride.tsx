...
const Ride: FC<Props> = ( { measurements, activeMeasurements, tripId, taskId} ) => {
        ...
        post( meas.query, { tripID: tripId, measurement: meas.queryMeasurement }, (res: any) => {            
            ...
            if ( meas.value )
            {
                if ( path.data.length === 0 )
                    return (
                        console.log("error here no data for speed")
                    );
    ...
    return (<> 
        ...
        {/* { <RoutingMachine path={routmachinePath}></RoutingMachine> } */}
        </>)
}
