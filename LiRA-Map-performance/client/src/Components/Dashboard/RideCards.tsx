...
const Cards: FC<CardsProps> = ( { metas, showMetas, onClick } ) => {  
    ...
    const renderRow: ListRowRenderer = ( { index, key, style } ): ReactNode => {
        ...
        const startCity = JSON.parse(meta.StartPositionDisplay).city
        const endCity = JSON.parse(meta.EndPositionDisplay).city

        //const{city}=meta.StartPositionDisplay
        return <div key={key} style={style}>
            
            {/*{console.log(typeof meta.StartPositionDisplay)}
            {console.log(byen)}*/}
            <Checkbox 
                fs={checked[n]}
                ...
                //html={<div><b>{meta.TaskId}</b><br></br>{byen} - {kommune}</div>}

                //html={<div><b>{meta.TaskId}</b><br></br>({new Date(meta.Created_Date).toLocaleDateString()})</div>}
                html={<div><b>{meta.TaskId}</b><br></br>{new Date(meta.Created_Date).toLocaleDateString()}<br></br>{startCity} - {endCity}</div>}
                //html={<div>{new Date(meta.Created_Date).toLocaleDateString()}<br></br>{byen} - {kommune}</div>}
